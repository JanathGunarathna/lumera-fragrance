...
export default function Cart() {
  ...
  if (loading) return <div className="max-w-5xl mx-auto px-6 py-24 text-muted">Loading your bag…</div>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid md:grid-cols-3 gap-12">
                ...
                <div key={item.id} className="flex gap-5 border-b border-white/10 pb-6">
                  <div className="w-24 h-24 bg-panel border border-white/10 flex items-center justify-center flex-shrink-0">
                    {item.product.imageUrl
                      ...
                      : <span className="font-display text-2xl text-gold">L</span>}
                  </div>
                  <div className="flex-1">
                    <h3 className="font-display text-xl text-cream">{item.product.name}</h3>
                    <p className="text-muted2 text-xs">{item.product.volume}</p>
                    <div className="flex items-center gap-3 mt-3">
                      <div className="flex items-center border border-white/20">
                        <button onClick={() => updateQuantity(item.id, Math.max(1, item.quantity - 1))} className="px-2 py-1 text-cream/90 hover:text-gold">−</button>
                        <span className="px-3 text-cream text-sm">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="px-2 py-1 text-cream/90 hover:text-gold">+</button>
                      </div>
                      <button onClick={() => handleRemove(item.id)} className="text-xs text-muted2 hover:text-gold uppercase tracking-widest">
                        Remove
                      </button>
                    </div>
                  </div>
          ...
          <div className="bg-panel border border-white/10 p-8 h-fit">
            ...
            <div className="flex justify-between text-cream/90 text-sm mb-3">
              <span>Subtotal</span><span>${cartTotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-cream/90 text-sm mb-6">
