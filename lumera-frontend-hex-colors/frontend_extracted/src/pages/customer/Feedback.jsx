...
export default function Feedback() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16">
      <p className="uppercase text-xs tracking-widest2 text-gold mb-2">Customer Voices</p>
      ...
      <form onSubmit={handleSubmit} className="border border-white/10 p-8 mb-16">
        ...
        <div className="mb-5">
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map(n => (
              <button type="button" key={n} onClick={() => setRating(n)} className={`text-2xl ${n <= rating ? 'text-gold' : 'text-muted2'}`}>★</button>
            ))}
          </div>
        </div>
        <div className="mb-5">
          <label className="block text-xs uppercase tracking-widest text-muted mb-2">Your Feedback</label>
          <textarea
            required
            rows={4}
            value={comment}
        ...
        {!isAuthenticated && <p className="text-muted2 text-xs mt-3">You'll need to sign in first.</p>}
      </form>
        ...
        {reviews.length === 0 ? (
          ...
          reviews.map(r => (
            <div key={r.id} className="border-b border-white/10 pb-6">
              ...
              <p className="text-cream/90 mt-2">{r.comment}</p>
              <p className="text-muted2 text-xs mt-2">{r.user?.fullName || 'Verified customer'} · {new Date(r.createdAt).toLocaleDateString()}</p>
            </div>
          ))
        )}
      </div>
